...
def inference(images):
  ...
  with tf.variable_scope('conv1') as scope:
    kernel = _variable_with_weight_decay('weights',
                                         ...
                                         wd=0.0)
    bind_num = 2
    conv = maxout_cnn_layer(images, kernel, bind_num, [1, 1, 1, 1], padding='SAME',name=None)
    #conv = tf.nn.conv2d(images, kernel, [1, 1, 1, 1], padding='SAME')
    biases = _variable_on_cpu('biases', [48], tf.constant_initializer(0.0))
    pre_activation = tf.nn.bias_add(conv, biases)
    conv1 = pre_activation
    #conv1 = tf.maximum(pre_activation, pre_activation*0.1)
  ...
  with tf.variable_scope('conv2') as scope:
    kernel = _variable_with_weight_decay('weights',
                                         ...
                                         wd=0.0)
    bind_num = 2
    conv = maxout_cnn_layer(pool1, kernel, bind_num, [1, 1, 1, 1], padding='SAME',name=None)
    #conv = tf.nn.conv2d(norm1, kernel, [1, 1, 1, 1], padding='SAME')
    biases = _variable_on_cpu('biases', [48], tf.constant_initializer(0.0))
    pre_activation = tf.nn.bias_add(conv, biases)
    conv2 = pre_activation
    #conv2 = tf.maximum(pre_activation, pre_activation*0.1)
  ...
  with tf.variable_scope('conv3') as scope:
    kernel = _variable_with_weight_decay('weights',
                                         ...
                                         wd=0.0)
    bind_num = 2
    conv = maxout_cnn_layer(conv2, kernel, bind_num, [1, 1, 1, 1], padding='SAME',name=None)
    #conv = tf.nn.conv2d(norm2, kernel, [1, 1, 1, 1], padding='SAME')
    biases = _variable_on_cpu('biases', [96], tf.constant_initializer(0.0))
    pre_activation = tf.nn.bias_add(conv, biases)
    conv3 = pre_activation
    #conv3 = tf.maximum(pre_activation, pre_activation*0.1)
  ...
  with tf.variable_scope('local3') as scope:
    ...
    bind_num = 2
    loc = tf.matmul(reshape, weights) 
    biases = _variable_on_cpu('biases', [256], tf.constant_initializer(0.1))
    pre_activation = maxout(loc, bind_num) + biases
    local3 = pre_activation
